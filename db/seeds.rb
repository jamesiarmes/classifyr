...
# Insert an initial data set if we're not in prod.
unless Rails.env == "production"
  dataset = DataSet.find_or_initialize_by(title: "Burlington, VT Police Incidents")
  dataset.update!(
    {
...
CSV.foreach(Rails.root.join("db", "import", "apco_common_incident_types_2.103.2-2019.csv"), headers: true) do |line|
  type = CommonIncidentType.find_or_initialize_by(code: line[0])
  type.update! description: line["description"], notes: line["notes"]
end
